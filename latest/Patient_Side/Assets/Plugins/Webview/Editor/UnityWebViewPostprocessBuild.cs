...
internal class AndroidManifest : AndroidXmlDocument {
    ...
    {
        bool changed = false;
        if (SelectNodes("/manifest/uses-permission[@android:name='android.permission.READ_EXTERNAL_STORAGE']", nsMgr).Count == 0)
        {
            var elem = CreateElement("uses-permission");
        ...
        if (SelectNodes("/manifest/uses-feature[@android:name='android.permission.WRITE_EXTERNAL_STORAGE']", nsMgr).Count == 0)
        {
            var elem = CreateElement("uses-permission");
